...
def generate_tsne_plot(filtered_data, show_edges=False, edge_threshold=2.0, dimension='2d', show_titles=False):
    ...
    for i, (_, row) in enumerate(filtered_data.iterrows()):
        ...
        base_color = "rgba(55,55,55,0.92)" if df_value == "signals" else hex_to_rgba_custom(color_map.get(cluster_id, "#ccc"), 0.89)
    ...
    shadow_colors = colors
    shadow_opacities = [0.07 for _ in range(len(colors))]
    ...
    try:
        if dimension == '3d':
            ...
            if len(coords) == 0:
                ...
                curated_colors = np.array(colors)[curated_mask.values] if np.sum(curated_mask.values) > 0 else []
