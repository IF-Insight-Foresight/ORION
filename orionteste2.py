...
# --- Apple-inspired style constants ---
...
most_common_words = Counter(" ".join(data["PreprocessedText"]).split()).most_common(50)
suggestions_words = [w for w, _ in most_common_words if len(w) > 2]
suggestions_tags = set()
for tags in data["Tags"].dropna():
    for t in str(tags).split(","):
        if len(t.strip()) > 2:
            suggestions_tags.add(t.strip())
suggestions_titles = [t for t in data["Title"].dropna() if len(t.split()) < 6]
search_suggestions = sorted(set(suggestions_words) | suggestions_tags | set(suggestions_titles))
...
def generate_tsne_plot(filtered_data, show_edges=False, edge_threshold=2.0, focus_id=None, spotlight_keyword=None, dimension='2d', show_titles=False):
    ...
    if dimension == '3d':
        ...
        fig.update_layout(
            ...
            scene=dict(
                aspectmode="data",
                xaxis=dict(visible=False, showgrid=False, zeroline=False, showticklabels=False, backgroundcolor="#181818"),
                yaxis=dict(visible=False, showgrid=False, zeroline=False, showticklabels=False, backgroundcolor="#181818"),
                zaxis=dict(visible=False, showgrid=False, zeroline=False, showticklabels=False, backgroundcolor="#181818"),
                bgcolor="#181818",
            ),
                                ...
                                children=[
                                    ...
                                    html.Div(
                                        ...
                                        children=[
                                            html.Div(
                                                ...
                                                children=[
                                                    ...
                                                    html.Span(
                                                        ...
                                                        title="Use quotes for phrases, AND/OR for logic, e.g.: \"climate change\" AND (AI OR blockchain)"
                                                    )
                                                ]
                                            ),
                                            html.Div(
                                                ...
                                                children=[
                                                    html.I(
                                                        className="fa fa-search",  # requires FontAwesome loaded globally
                                                        style={
                                                            "position": "absolute",
                                                            "left": "14px",
                                                            "top": "50%",
                                                    ...
                                                    dcc.Input(
                                                        ...
                                                        style={
                                                            **APPLE_INPUT,
                                                            "width": "100%",
                                                            "paddingLeft": "38px",  # icon space
                                                            "fontSize": "16px",
                                                            "boxShadow": "0 1px 6px rgba(0,0,0,0.10)",
                                                            "marginBottom": "0"
                                                        }
                                                    ),
                                                    dcc.Store(id='search-suggestions-store', data=search_suggestions),
                                                    html.Datalist(id='search-suggestions', children=[]),
                                                ]
                                            ),
                                            html.Div(
                                                style={"display": "flex", "gap": "10px", "marginTop": "14px", "justifyContent": "center"},
                                                children=[
                                                    dbc.Button(
                                                        "Clear Focus",
                                                        id="clear-focus-btn",
                                                        color="secondary",
                                                        style={**APPLE_BUTTON_SECONDARY, "fontSize": "13px", "padding": "6px 14px", "backgroundColor": "#222"}
                                                    ),
                                                    dbc.Button(
                                                        "Clear Spotlight",
                                                        id="clear-spotlight-btn",
                                                        color="secondary",
                                                        style={**APPLE_BUTTON_SECONDARY, "fontSize": "13px", "padding": "6px 14px", "backgroundColor": "#222"}
                                                    )
                                                ]
                                            ...
                                            dbc.Tooltip(
                                                "Use quotes for phrases. Use AND/OR for advanced queries. Example: \"climate change\" AND (AI OR blockchain)",
...
def update_tsne_plot(
    ...
    # Filtering logic (copied from original app, simplified for demonstration)
    filtered_data = data
    # Apply search term filtering
    if search_term and search_term.strip():
        mask = data["PreprocessedText"].apply(lambda txt: match_advanced_query(txt, search_term))
        filtered_data = filtered_data[mask]
    # Apply cluster highlight filtering (if not -1)
    ...
    # Apply driving force filter (if not "(All)")
    ...
    # By default, only show curated driving forces unless toggle is checked
...
#############################################
# t-SNE PLOT + TABLE UPDATE CALLBACK
#############################################
...
#         Output('current-subset', 'data'),
#         Output('show-edges', 'value'), 
#     ],
#     [
#         Input('apply-filters-button', 'n_clicks'),
#         Input('reset-filters-button', 'n_clicks'),
#         Input('show-edges', 'value'),     # Add input to read toggle
#     ],
#     [
#         State('search-term', 'value'),
#         State('cluster-highlight', 'value'),
#         State('driving-force-filter', 'value'),
#         State('current-subset', 'data'),
#     ],
#     prevent_initial_call=False
# )
# def update_tsne_and_explorer(
#     apply_clicks, reset_clicks, show_edges,   # 3 Inputs
#     search_term, cluster_val, driving_forces, stored_subset  # 4 States
# ):
#     # Apply filters to the dataset
#     filtered = data.copy()
#
#     # Keyword search only (semantic option removed)
#     if search_term and search_term.strip():
#         mask = filtered["PreprocessedText"].apply(lambda t: match_advanced_query(t, search_term))
#         filtered = filtered[mask]
#
#     if cluster_val != -1:
#         filtered = filtered[filtered["Cluster"] == cluster_val]
#     if driving_forces and "(All)" not in driving_forces:
#         filtered = filtered[filtered["Driving Force"].isin(driving_forces)]
#
#     # Only show t-SNE (no viz_type/word_graph)
#     fig = generate_tsne_plot(filtered, show_edges=("edges" in (show_edges or [])))
#
#     # Optionally hide feedback or keep empty for clean Apple look
#     feedback = ""
#
#     # Prepare table data
#     tbl = filtered[["ID", "Title", "Description", "Driving Force"]].copy()
#     tbl["ShortDescription"] = tbl["Description"].apply(
#         lambda d: " ".join(str(d).split()[:10]) + "…" if isinstance(d, str) and len(d.split()) > 10 else d
#     )
#     records = tbl[["ID", "Title", "ShortDescription", "Driving Force"]].to_dict("records")
#     tooltips = [{"ShortDescription": {"value": row["Description"] or "", "type": "text"}} for _, row in tbl.iterrows()]
#     new_subset = tbl["ID"].unique().tolist()
#
#     return (
#         fig, records, tooltips,
#         search_term, cluster_val, driving_forces, new_subset, show_edges
#     )
